...
<p>
리눅스에는 시스템 최적화를 위한 다양한 툴이 제공됩니다. 엘라스틱서치가 효율적으로 동작하기 위한 부분을 최적화하는 방법에 대해 설명하도록 하겠습니다.
</p>
<br>
...
ulimit 명령어나 sysctl 명령어를 참고하면 리소스들의 정보를 확인하고 이에 따른 애플리케이션의 성능도 미리 예측할 수 있습니다.

<br>

### ulimit

ulimit 명령어는 애플리케이션이 실행될 떄 얼마만큼의 리소스를 할당받을 수 있는지에 대한 전반적인 리소스 관리를 수행합니다. 명령어 수행방법은 아래와 같습니다.
...
ulimit 값을 변경하는 작업은 신중하게 진행하여야 합니다. 시스템의 특성이나 처리하는데 데이터 종류에 따라 리소스 제한을 변경해야 좀 더 좋은 성능을 발휘하는 경우도 있겠지만 일반적인 경우는 기본 설정갓을 사용하는 것만으로도 충분할 때가 많습니다. ulimit은 내부적으로 소프트 설정과 하드 설정을 가지고 있습니다. 소프트 설정은 프로세스가 실행될 때 최초로 할당받는 값이고, 하드설저은 운영 중 리소스의 한계에 도달했을 경우 추가적으로 할당받을 수 있는 값입니다.

<br>

### sysctl

sysctl 명령어는 리눅스 내부에 존재하는 커널의 파라미터를 조절할 수 있도록 설정 값을 제공합니다. 이를 이용해 커널 레벨의 다양한 파라미터들을 확인할 수 있습니다.
...
sysctl 명령어는 커널에서 제공하는 다양한 파라미터 정보를 제공합니다. 각 항목 하나하나가 커널에서 사용 중인 중요한 정보이기 때문에 함부로 수정할 경우 자칫 운영체제 전체가 심각한 불능 상태에 빠질수도 있기 때문에 수정할 때 항상 주의하여야 합니다.

<br>

### 실행중인 애플리케이션의 리소스 확인

애플리케이션마다 리소스 제한이 있다는 사실을 알고 나면 모든 애플리케이션이 실제로 설정한 값만큼의 리소스만 할당받는 것인지 궁금할 것입니다. 엘라스틱서치를 실행하고 있는 PID를 이용해 프로세스 정보를 조회해보도록 하겠습니다.
...
프로세스의 PID 값을 찾았으면 그 PID 값을 이용해 프로세스 리소스를 확인해보도록 하겠습니다.

```
cat /proc/4941/limits
```
...
엘라스틱서치에서 효율적인 서버 운영을 위해 가이드를 제공합니다. 이를 통해 어떻게 해야 엘라스틱서치 클러스터를 좀 더 안정적이고 효율적으로 운영할 수 있을지에 대해 알아보겠습니다.

1. Max Open File 한계 설정

```
vi /etc/security/limits.conf # 해당 파일을 편집
